...
class UpdWindow(QDialog):
    url = ""
    gitcodeurl = None
    githuburl = "https://github.com/a191442029/HeartBeat"
    @try_except("下载窗口初始化")
    def __init__(self, parent=None):
        ...
        # clicked 连接统一放到下方单槽分发处(避免与_on_download_clicked重复触发)
        ...
        self._download_done = False  # 状态机: 下载完成后按钮变"重启以应用更新"

        # 按钮统一单槽分发, 避免download_finished里重复connect导致多次触发
    ...
    def start_download(self):
        ...
        # 保存到程序所在目录(与start_update_program查找upd.exe的路径一致; 原相对路径受CWD影响)
        if getattr(sys, 'frozen', False) or hasattr(sys, "_MEIPASS"):
            base_dir = os.path.dirname(sys.executable)
        else:
            base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # UI/上级=项目根
        save_path = os.path.join(base_dir, "upd.exe")
        logger.info(f"更新文件保存路径: {save_path}")
    ...
    def _on_download_clicked(self):
        """下载按钮单槽分发: 未完成→开始/重新下载; 已完成→重启应用更新"""
