...
## 最新更新 v1.1.4（2026-09-25）

- **体积优化**：ffmpeg 从 EXE 分离，安装包从 146MB 降至约 50MB（下载快 3 倍）
- **ffmpeg 独立部署**：从 Release 下载 `ffmpeg.exe`，放入程序目录 `bin\` 下即可（`find_ffmpeg` 自动定位：EXE 同目录 `bin\` → PATH）
- 含 v1.1.3 全部修复（启动扫描 + 旧版 Win10 扫描兼容）；功能与 v1.1.1 一致

## 历史更新 v1.1.3（2026-09-25）

- **启动早期扫描修复**：修复启动后立即扫描报 "no running event loop"、设备列表空白的问题（bleak MTA 检查在事件循环未就绪时自动跳过）
- 含 v1.1.2 全部修复（旧版 Win10 扫描兼容）；功能与 v1.1.1 一致，仅桌面端 EXE 更新

## 历史更新 v1.1.2（2026-09-25）

- **旧版 Win10 扫描修复**：修复 Windows 10 2004 以下版本扫描 BLE 设备报 "property is not available in this version of Windows" 的问题（bleak 扩展广播属性在旧系统自动跳过，手环扫描不受影响）
- 功能与 v1.1.1 一致，仅桌面端 EXE 更新；安卓/鸿蒙接收端沿用 v1.1.1

## 历史更新 v1.1.1（2026-09-25）
...
**方法 2**（推荐）：下载编译好的程序 —— 前往 [Releases 页面](https://github.com/a191442029/HeartBeat/releases/latest) 下载 `HRMLink-v1.1.2.exe`，双击运行
- 优点：门槛低，单文件双击即可运行
- 缺点：相对不太透明，exe 体积较大
