...
def checkupdate() :
    ...
    urlRaw = "https://raw.githubusercontent.com/a191442029/HeartBeat/main/version.json"
    urlApi = "https://api.github.com/repos/a191442029/HeartBeat/contents/version.json?ref=main"
    # Sequential fallback: return on first success (old finally+return overwrote earlier success)
    for name, fn, u in (("github-raw", check_with_raw, urlRaw),
                        ("github-api", check_with_githubapi, urlApi)):
        try:
            return fn(u)
        except Exception as e:
            logger.warning(f"Update check failed ({name}): {e}", exc_info=True)
    return False, 'Failed', '', '', ''
