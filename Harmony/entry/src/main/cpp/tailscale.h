...
/**
 * An SSH channel stream (shell) handle.
 */
typedef struct ts_ssh_channel ts_ssh_channel;

/**
 * An SSH session handle.
 */
typedef struct ts_ssh_session ts_ssh_session;
...
/**
 * Connect to an SSH server through the Tailscale network stack.
 *
 * `dev`        - Tailscale device handle
 * `remote`     - target address (e.g. "100.76.21.21:22")
 * `username`   - SSH username (NUL-terminated C string)
 * `password`   - SSH password (NUL-terminated C string)
 *
 * Returns null on error.
 */
struct ts_ssh_session *ts_ssh_connect(const struct ts_device *dev,
                                      const struct ts_sockaddr *remote,
                                      const char *username,
                                      const char *password);

/**
 * Open an interactive shell channel on the SSH session.
 *
 * Returns null on error.
 */
struct ts_ssh_channel *ts_ssh_open_shell(const struct ts_ssh_session *session);

/**
 * Read data from the SSH channel (blocking).
 *
 * Returns the number of bytes read, or a negative number on error/EOF.
 *
 * # Safety
 *
 * `buf` must be safe to convert into a mutable Rust slice of length `len`.
 */
int ts_ssh_channel_read(struct ts_ssh_channel *channel, uint8_t *buf, uintptr_t len);

/**
 * Write data to the SSH channel (blocking).
 *
 * Returns the number of bytes written, or a negative number on error.
 *
 * # Safety
 *
 * `buf` must be safe to convert into a Rust slice of length `len`.
 */
int ts_ssh_channel_write(struct ts_ssh_channel *channel, const uint8_t *buf, uintptr_t len);

/**
 * Close an SSH channel.
 */
void ts_ssh_channel_close(struct ts_ssh_channel *channel);

/**
 * Disconnect an SSH session.
 */
void ts_ssh_disconnect(struct ts_ssh_session *session);
...
/**
 * Bridge a TUN file descriptor to the Tailscale data plane.
 *
 * After calling this, IP packets read from `tun_fd` will be sent through
 * WireGuard to tailnet peers, and packets from peers will be written to
 * `tun_fd`. This enables system-wide VPN integration.
 *
 * Returns 0 on success, -1 on error.
 */
int ts_vpn_bridge(struct ts_device *dev, int tun_fd);

/**
 * List all peers in the tailnet as a JSON array string.
 *
 * Returns a heap-allocated C string (caller must free with ts_free_string), or NULL on error.
 * JSON format: [{"hostname":"...","ipv4":"100.x.x.x","online":true},...]
 */
char *ts_peers(const struct ts_device *dev);

/**
 * Free a string returned by ts_peers.
 */
void ts_free_string(char *ptr);
